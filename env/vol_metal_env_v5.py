"""
V5 Metals Environment with News Sentiment Features

Adds 15 news sentiment features (per metal: raw, ema5, shock) on top of V4's 41 features.
Total: ~56 features. News captures regime-shifting events that pure technical indicators miss.
"""
...
def load_news_sentiment(path: str = "data/news_sentiment_daily.csv") -> pd.DataFrame:
    if not Path(path).exists():
        LOG.warning(f"News sentiment file not found: {path}")
        return pd.DataFrame()
    df = pd.read_csv(path)
    if "time" in df.columns:
...
class VolMetalEnvV5(gym.Env):
    ...
    def __init__(
        ...
        for name, df in metal_dfs.items():
            ...
            pv = feats["parkinson_vol"]
            feats["vol_of_vol"] = pv.rolling(24).std().fillna(0) / (pv.rolling(24).mean().fillna(1e-12) + 1e-12)
        ...
        if news_df is not None and not news_df.empty:
            news_cols = [c for c in news_df.columns if c.startswith("news_")]
            for name in self.feat_dfs:
                times = pd.to_datetime(self.raw_dfs[name]["time"])
                n = len(self.feat_dfs[name])
                news_aligned = news_df[news_cols].reindex(times, method="ffill").fillna(0.0)
                self.feat_dfs[name] = pd.concat(
                    [self.feat_dfs[name].reset_index(drop=True), news_aligned.iloc[:n].reset_index(drop=True)],
                    axis=1,
                )
        ...
        LOG.info(f"V5 features: {self.n_features}")
    ...
    def step(self, action):
        ...
        feats = self.feat_dfs[self.current_metal]
        t_safe = min(self.t, len(feats) - 1)

        vol_change = float(feats["vol_ratio_5"].iloc[t_safe])
        ...
        range_feature = float(feats["range_pct"].iloc[t_safe])
        vol_risk_premium = 0.0
        if abs(target_pos) > 0.3 and range_feature > 0.01:
            vol_risk_premium = 0.0005 * range_feature * 100

        news_col = f"news_{self.current_metal}"
        news_alignment = 0.0
        if news_col in feats.columns:
            news_val = float(feats[news_col].iloc[t_safe])
            if abs(news_val) > 0.1 and abs(target_pos) > 0.1:
                if np.sign(target_pos) == np.sign(news_val):
                    news_alignment = 0.002 * abs(news_val)
                else:
                    news_alignment = -0.003 * abs(news_val)
        ...
        reward = direction_pnl + vol_prediction_reward + vol_risk_premium + news_alignment + hold_penalty + flip_penalty + inactivity_penalty - cost
