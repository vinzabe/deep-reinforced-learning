"""
Metal Volatility Strategy - A different approach to metals profitability.

Instead of predicting direction (which is nearly random on daily timeframe),
this strategy exploits the PREDICTABILITY of VOLATILITY:
- Metals have persistent volatility regimes (high-vol clusters, low-vol clusters)
- VIX-like volatility risk premium exists in metals
- Selling volatility (selling options) has historically been profitable

The RL agent learns WHEN to be in the market (high-vol regimes = more opportunity)
and POSITION SIZING (bigger in high-vol, smaller in low-vol).

Key insight from research (Bollerslev & Todorov 2011):
- Volatility is much more predictable than returns
- Strategies that trade volatility (not direction) have higher Sharpe ratios

This env rewards the agent for:
1. Correct volatility forecasting (predict next-day vol)
2. Correctly timing entries (entering before vol expands)
3. Position sizing based on predicted vs actual vol
"""
...
class VolatilityMetalEnv(gym.Env):
    """
    The agent trades metal volatility, not direction.

    Action: continuous [-1, 1] = conviction score
    - Positive = expect vol to increase (buy straddle-like)
    - Negative = expect vol to decrease (sell straddle-like)

    Reward based on:
    - How well position aligns with actual volatility change
    - Volatility risk premium capture
    - PnL from realized vs implied volatility spread
    """
    ...
    def step(self, action):
        ...
        info = {
            ...
            "vol_prediction_reward": vol_prediction_reward,
        }
